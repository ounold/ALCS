...
class PhaseConfigCPU3:
    ...
    def validate(self, phase_name: str) -> "PhaseConfigCPU3":
        if self.episodes < 0:
...
class ExperimentConfigCPU3:
    ...
    phases: Dict[str, PhaseConfigCPU3]
    steps_to_goal_threshold: Optional[int] = None
    global_total_episodes: Optional[int] = None

    def validate(self) -> "ExperimentConfigCPU3":
        ...
        for phase_name in ("explore", "exploit1", "exploit2"):
            ...
            total_episodes += self.phases[phase_name].validate(phase_name).episodes
        if total_episodes <= 0:
...
def load_yaml_defaultsCPU3(path: str) -> Dict[str, Any]:
...
def build_arg_parserCPU3(defaults: Mapping[str, Any]) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="ACS2 Experiment Runner (CPU3)")
    ...
    general.add_argument("--alp_mark_only_incorrect", type=parse_bool, default=defaults.get("alp_mark_only_incorrect", DEFAULT_EXPERIMENT_VALUES["alp_mark_only_incorrect"]))
    general.add_argument("--no_subsumption", type=parse_bool, nargs="?", const=True, default=defaults.get("no_subsumption", False))
    ...
    env_group.add_argument("--reset_to_random_start", type=parse_bool, default=environment_defaults.get("reset_to_random_start", True))
    ...
    env_group.add_argument("--is_slippery", type=parse_bool, default=params_defaults.get("is_slippery"))
    ...
    for phase in ("explore", "exploit1", "exploit2"):
        ...
        phase_group.add_argument(f"--{phase}_alp", type=parse_bool, default=defaults.get(f"{phase}_alp", DEFAULT_EXPERIMENT_VALUES[f"{phase}_alp"]))
        phase_group.add_argument(f"--{phase}_ga", type=parse_bool, default=defaults.get(f"{phase}_ga", DEFAULT_EXPERIMENT_VALUES[f"{phase}_ga"]))
        phase_group.add_argument(f"--{phase}_decay", type=parse_bool, default=defaults.get(f"{phase}_decay", DEFAULT_EXPERIMENT_VALUES[f"{phase}_decay"]))
...
def experiment_config_from_argsCPU3(args: argparse.Namespace) -> ExperimentConfigCPU3:
    env_args = {
