...
class Classifier:
    ...
    def __init__(self, condition: List[str], action: int, effect: List[str], cfg: ACS2Configuration, time_created: int = 0, origin_source: str = "unknown", creation_episode: int = 0):
        ...
        self._C: List[str] = condition
        self.A: int = action
        self._E: List[str] = effect
    ...
    @property
    def key(self) -> Tuple:
        """ Returns a unique key for the classifier based on bitmasks and action. """
        return (self.condition_bits, self.wildcard_mask, self.A, self.effect_bits, self.effect_wildcard_mask)

    def sync_from_bits(self):
        """ Synchronizes string representations C and E from bitmasks. """
        new_C = ['#' for _ in range(self.cfg.l_len)]
        for i in range(self.cfg.l_len):
            shift = i * 8
            if (self.wildcard_mask >> shift) & 0xFF:
                new_C[i] = str((self.condition_bits >> shift) & 0xFF)
        self._C = new_C

        new_E = ['#' for _ in range(self.cfg.l_len)]
        for i in range(self.cfg.l_len):
            shift = i * 8
            if (self.effect_wildcard_mask >> shift) & 0xFF:
                new_E[i] = str((self.effect_bits >> shift) & 0xFF)
        self._E = new_E
    ...
    @E.setter
    def E(self, value: List[str]):
        self._E = value

    def __repr__(self) -> str:
        self.sync_from_bits() # Ensure strings are correct for printing
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return self.condition_bits == other.condition_bits and \
               self.wildcard_mask == other.wildcard_mask and \
               self.A == other.A and \
               self.effect_bits == other.effect_bits and \
               self.effect_wildcard_mask == other.effect_wildcard_mask
