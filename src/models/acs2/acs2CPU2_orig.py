...
from .confCPU2_orig import ACS2Configuration
from .classifierCPU2_orig import Classifier
from . import logicCPU2_orig as logic
...
class ACS2:
    ...
    def __init__(self, cfg: ACS2Configuration):
        self.cfg = cfg
        self.population_dict: Dict[Tuple, Classifier] = {}
        self.population_by_action: Dict[int, List[Classifier]] = {a: [] for a in range(cfg.num_actions)}
        self.time: int = 0
        self.curr_ep_idx: int = 0

    @property
    def population(self) -> List[Classifier]:
        """ Returns the population as a list. """
        return list(self.population_dict.values())

    @staticmethod
    def state_to_bits(state: List[str]) -> int:
        """ Converts a string-based state list to a single integer bitmask. """
        state_bits = 0
        for i, sym in enumerate(state):
            state_bits |= (int(sym) << (i * 8))
        return state_bits
    ...
    def run_step(self, env_state: List[str], explore: bool = True) -> (int, List[Classifier]):
        ...
        match_set = [cl for cl in self.population_dict.values() if cl.matches_bits(state_bits)]

        if not match_set:
            match_set = logic.generate_covering_classifiers(self, env_state)
            for cl in match_set:
                self.add_to_population(cl)
    ...
    def apply_learning(self, action_set: List[Classifier], prev_state: List[str], action: int, reward: float, next_state: List[str], is_terminal: bool = False, curr_ep_idx: int = 0):
        ...
        next_state_bits = self.state_to_bits(next_state)
        next_match_set = [cl for cl in self.population_dict.values() if cl.matches_bits(next_state_bits)]
    ...
    def _apply_simple(self, action_set: List[Classifier], prev_state: List[str], action: int, reward: float, next_state: List[str], is_terminal: bool = False):
        ...
        next_state_bits = self.state_to_bits(next_state)
        next_match = [cl for cl in self.population_dict.values() if cl.matches_bits(next_state_bits)]
        ...
        for cl in action_set[:]: 
            ...
            if cl.get_anticipation(prev_state) == current_state:
                ...
                if cl.q < self.cfg.theta_i:
                    self.remove_from_population(cl)
    ...
    def add_to_population(self, cl: Classifier):
        ...
        """
        # Check for identical classifiers (O(1) lookup)
        key = cl.key
        if key in self.population_dict:
            self.population_dict[key].num += 1
            return

        # Check for subsumption by an existing classifier (O(N_action) lookup)
        if self.cfg.do_subsumption:
            for ex in self.population_by_action[cl.A]:
                if logic.does_subsume(ex, cl, self.cfg):
                    ex.num += 1
                    return
        
        self.population_dict[key] = cl
        self.population_by_action[cl.A].append(cl)

    def remove_from_population(self, cl: Classifier):
        """ Removes a classifier from both indexed structures. """
        key = cl.key
        if key in self.population_dict:
            del self.population_dict[key]
            # O(N_action) removal from list, but much better than full population
            if cl in self.population_by_action[cl.A]:
                self.population_by_action[cl.A].remove(cl)

    def copy(self) -> 'ACS2':
        """ Creates a lightweight copy of the ACS2 agent. """
        import copy
        # cfg is a dataclass, so we can use copy.replace or just deepcopy it once
        new_agent = ACS2(copy.deepcopy(self.cfg))
        # Re-build indexed structures
        for cl in self.population_dict.values():
            cl_copy = cl.copy()
            new_agent.population_dict[cl_copy.key] = cl_copy
            new_agent.population_by_action[cl_copy.A].append(cl_copy)
