...
from .confGPU3 import ACS2Configuration
...
class ACS2GPU:
    ...
    def batch_add_classifiers(self, cand_list):
        ...
        if torch.any(to_add_mask):
            # Still loop over agents for adding new slots because it involves finding free slots
            # and managing population size per agent, which is inherently per-agent.
            # But we only loop over agents that have something to add.
